...
// Current file has "Components" and "Details" JSX inside detailsContainer even though tabBar renders "Vocab" and "Sentences"
// Let's replace the whole detailsContainer to be exactly what was requested.

const startMarker = "<View style={styles.detailsContainer}>";
const endMarker = "</View>\n    </ScrollView>";

const oldContent = file.substring(file.indexOf(startMarker), file.indexOf(endMarker) + "</View>".length);

const newContent = `<View style={styles.detailsContainer}>
        {activeTab === 'Meanings' && (
            ...
            {words.length === 0 ? (
              ...
              words.map((item: any, idx: number) => (
                <TouchableOpacity 
                  ...
                  onPress={() => router.push({ pathname: '/word/[id]', params: { id: item.rowid?.toString() ?? item.id?.toString() } } as any)}
                >
                  ...
                  <View style={styles.wordCenter}>
                    ...
                    <Text style={styles.wordMeaning} numberOfLines={1}>{item.meanings.split('/')[0]}</Text>
                  </View>
                  <View style={styles.wordRight}>
                    {/* HSK Badge could go here if item.level exists */}
                  </View>
                </TouchableOpacity>
              ))
            )}
...
// Add new styles
const newStyles = `  vocabContainer: { flex: 1 },
  ...
  wordRight: { width: 40, alignItems: 'flex-end' },
  sentencesContainer: { flex: 1 },
  ...
  sentenceHighlight: { color: Colors.primary, fontWeight: '700' },
